...
WHEN the user executes `gitlab-ci-cli pipeline explain --jobs build,test,deploy`
...
WHEN the user executes `gitlab-ci-cli pipeline explain --jobs all`
...
WHEN the user executes `gitlab-ci-cli pipeline explain --jobs build`
...
WHEN the user executes `gitlab-ci-cli pipeline trace --var CI_COMMIT_REF_NAME`
...
WHEN the user executes `gitlab-ci-cli pipeline trace --var MY_VAR`
THEN the CLI shows:
...
WHEN the user executes `gitlab-ci-cli pipeline trace --var INCLUDED_VAR`
...
WHEN the user executes `gitlab-ci-cli pipeline trace --var UNDEFINED_VAR`
...
WHEN the user executes `gitlab-ci-cli pipeline stages`
...
WHEN the user executes `gitlab-ci-cli pipeline stages --mermaid`
...
WHEN the user executes `gitlab-ci-cli pipeline includes`
...
GIVEN a valid `.gitlab-ci.yml` file
WHEN the user executes `gitlab-ci-cli pipeline summary`
...
| Command | Description |
|---------|-------------|
| `gitlab-ci-cli pipeline explain --jobs <list>` | Show job dependency graph in Mermaid |
| `gitlab-ci-cli pipeline explain --jobs <list> --json` | Dependency graph as JSON |
| `gitlab-ci-cli pipeline trace --var <name>` | Trace variable usage across pipeline |
| `gitlab-ci-cli pipeline trace --var <name> --json` | Variable trace as JSON |
| `gitlab-ci-cli pipeline stages` | List stages and their jobs |
| `gitlab-ci-cli pipeline stages --mermaid` | Stages as Mermaid diagram |
| `gitlab-ci-cli pipeline includes` | Show include hierarchy |
| `gitlab-ci-cli pipeline includes --json` | Include hierarchy as JSON |
| `gitlab-ci-cli pipeline summary` | Generate pipeline summary |
