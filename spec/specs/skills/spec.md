...
WHEN the agent executes `gitlab-ci-cli skills list`
...
WHEN the agent executes `gitlab-ci-cli skills list --json`
...
the CLI SHALL output complete workflow instructions for using gitlab-ci-cli to browse, validate, and generate GitLab CI pipelines.

#### Scenario: Get core skill content

GIVEN the CLI is installed with skill data
WHEN the agent executes `gitlab-ci-cli skills get core`
...
GIVEN the CLI is installed with skill data
WHEN the agent executes `gitlab-ci-cli skills get core --full`
...
WHEN the agent executes `gitlab-ci-cli skills get templates`
...
WHEN the agent executes `gitlab-ci-cli skills get nonexistent`
THEN the CLI displays an error message "Skill 'nonexistent' not found"
AND suggests available skills via `gitlab-ci-cli skills list`
...
WHEN the agent executes `gitlab-ci-cli skills path core`
...
GIVEN the CLI is installed
WHEN the agent executes `gitlab-ci-cli skills path`
...
WHEN the agent executes `gitlab-ci-cli skills get --all`
...
| `gitlab-ci-cli skills list --json` | List skills as JSON |
| `gitlab-ci-cli skills get <name>` | Get skill content |
| `gitlab-ci-cli skills get <name> --full` | Include full reference |
| `gitlab-ci-cli skills get --all` | Output every skill |
| `gitlab-ci-cli skills path [name]` | Print skill directory path |
